...
fun ProductListScreen(
    ...
    OnLoadProducts: (filters: B2CProductFilterDTO, page: Int, pageSize: Int) -> Unit = { _, _, _ -> },
    onBackClick: () -> Unit = {},
    ...
    var selectedSortOption by remember { mutableStateOf(BBLocalization.Current.Get(key = "d02a9b8b-001b-4e99-9073-9150016441f3", fallback = "Öne çıkan")) }
    var favoriteProductIds by remember { mutableStateOf<Set<Int>>(emptySet()) }

    val initialFilters = remember(languageId, productCategoryId, categoryIds) {
        B2CProductFilterDTO(
    ...
    LaunchedEffect(languageId, productCategoryId, categoryIds) {
        OnLoadProducts(initialFilters, 1, 50)
    }

    val products = State.ProductListData?.Products2?.Items.orEmpty()

    val filteredProducts = remember(products, searchText, selectedSortOption) {
        val searchFilteredProducts = if (searchText.isBlank()) {
            products
        } else {
            products.filter { product ->
                product.ProductName.contains(other = searchText, ignoreCase = true) ||
                        product.CategoryName.contains(other = searchText, ignoreCase = true) ||
                        product.BrandName.contains(other = searchText, ignoreCase = true)
            }
        }

        when (selectedSortOption) {
            BBLocalization.Current.Get(key = "c4fa944e-eb81-40dd-80f6-4209c78db57c", fallback = "En düşük fiyat") -> {
                searchFilteredProducts.sortedBy { product -> product.Price }
            }

            BBLocalization.Current.Get(key = "a6913ebd-df97-42c0-9fa5-2138d2cd73e1", fallback = "En yüksek fiyat") -> {
                searchFilteredProducts.sortedByDescending { product -> product.Price }
            }

            BBLocalization.Current.Get(key = "6788b820-f4b2-470b-92f8-7a8470387d4e", fallback = "Yeni gelenler") -> {
                searchFilteredProducts.sortedByDescending { product -> product.InsertedDate }
            }
    ...
    Scaffold(
        ...
        Column(
            ...
            RetailProductListHorizontalFilters(
                ...
                onItemClick = { selectedSortOption = it }
            )

            RetailProductListResultHeader(
                productCount = State.ProductListData?.Products2?.TotalItemCount ?: filteredProducts.size,
            ...
            when {
                ...
                !State.ErrorMessage.isNullOrBlank() && products.isEmpty() -> {
                ...
                filteredProducts.isEmpty() -> {
                    ...
                    BbProductGrid(
                        ...
                        items(
                            ...
                            key = { product -> "${product.ProductId}-${product.VariantId}-${product.StoreId}" }
                        ) { product ->
                            val isFavorite = favoriteProductIds.contains(product.ProductId)

                            BbProductCard(
                                product = product.ToProductCardModel(isFavorite = isFavorite),
                                onClick = {
                                    onProductDetailClick(product.ProductId, product.StoreId, product.VariantId)
                                },
                                onFavoriteClick = {
...
private fun RetailProductListResultHeader(
    ...
    androidx.compose.foundation.layout.Row(
        ...
        Column(
            ...
            Text(
                text = BBLocalization.Current.Get(key = "6cf7b92f-05e7-4ac7-be8c-ce98d8bf20c5", fallback = "Tüm Ürünler"),
...
private fun RetailProductListLoading() {
    Column(
        ...
        Text(
            text = BBLocalization.Current.Get(key = "0953041e-609f-40de-90b7-ced652c5cb95", fallback = ""),
...
private fun RetailProductListError(
    ...
    Column(
        ...
        Text(
            text = BBLocalization.Current.Get(key = "0a4c4d79-c42d-473a-8f38-23d57c89bbbe", fallback = "Ürünler yüklenemedi"),
...
private fun B2CProductData.ToProductCardModel(
    ...
    return BbProductCardModel(
        ...
        PriceText = FormatProductPrice(price = Price, currencySymbol = CurrencySymbol),
        OldPriceText = "",
        BadgeText = "",
        RatingText = "",
        CargoText = "",
